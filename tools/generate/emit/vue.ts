// Emitter for @sargamicons/vue: tree-shakeable Vue 3 functional components
// built on a shared render-function factory, so each icon file stays tiny.

import { promises as fs } from 'node:fs';
import path from 'node:path';
import { parseAll, ensureCleanDir, VARIANTS, PACKAGES_ROOT, type Icon, type Variant } from './parse.ts';

const SRC = path.join(PACKAGES_ROOT, 'vue', 'src');

const FACTORY = `import { h } from 'vue';
...
function iconFile(icon: Icon): string {
  return `import { createIcon } from '../_svg.js';\nexport default createIcon(${JSON.stringify(icon.component)}, ${JSON.stringify(icon.viewBox)}, ${JSON.stringify(icon.inner)});\n`;
}
...
async function main(): Promise<void> {
  ...
  console.log(`vue: ${all.Line.length} icons \u00d7 ${VARIANTS.length} variants written`);
}
