...
test('each component themes with currentColor and forwards props', async () => {
  const sample = await fs.readFile(path.join(SRC, 'Line', 'SiFlowLine.svelte'), 'utf8');
  expect(sample).toContain('currentColor');
  expect(sample).not.toContain('#000');
  expect(sample).toContain('$props()');
  expect(sample).toContain('{...rest}');
  expect(sample).toContain('{@html inner}');
});
